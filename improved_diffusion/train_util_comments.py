...
class TrainLoop:
    def __init__(  # need to take parameters also for each corresponding function in lightning
        self,
        *,
        model,
        diffusion,
        ...
        self.sync_cuda = th.cuda.is_available() # delete lighning

        self._load_and_sync_parameters() #checkpoints
        if self.use_fp16:
            self._setup_fp16()

        self.opt = AdamW(self.master_params, lr=self.lr, weight_decay=self.weight_decay) #@configure_optimizers
        if self.args.resume_id != '':   # checkpoints
            self._load_optimizer_state() # checkpoints
            # Model was resumed, either due to a restart or a checkpoint
            # being specified at the command line.
            self.ema_params = [
                self._load_ema_parameters(rate) for rate in self.ema_rate # checkpoints
            ]
        else:
            self.ema_params = [
        ...
        if th.cuda.is_available(): # delete lighning
            self.use_ddp = True
            self.ddp_model = DDP(
                self.model,
    ...
    def _setup_fp16(self):
        self.master_params = make_master_params(self.model_params) # Copy model parameters into a (differently-shaped) list of full-precision parameters.
    ...
    def run_loop(self):  # runs script in video_train.py
    ...
    def run_step(self): # part of run_loop
        t0 = time()
        self.forward_backward()  # IMPORTANT PART!!!!!!! -> lightning
        if self.use_fp16:
            self.optimize_fp16() #@configure_optimizers 
        else:
            self.optimize_normal() #@configure_optimizers
        self.log_step() #logger
    ...
    def forward_backward(self): # part of run_step, IMPORTANT PART!!!!!!! -> lightning
        ...
        for i in range(0, batch1.shape[0], self.microbatch): # automatic by lightning, drop
            ...
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev()) #@configure_optimizers
            ...
            if self.use_fp16:
                ...
                (loss * loss_scale).backward() # automatic by lightning, drop, but how to do distinguishement then ????
            else:
                loss.backward()
    ...
    def optimize_fp16(self):  #@configure_optimizers
    ...
    def optimize_normal(self): #@configure_optimizers
        self._log_grad_norm()
        self._anneal_lr()
        self.opt.step()
        for rate, params in zip(self.ema_rate, self.ema_params):
    ...
    def save(self): # saving checkpoints
        if dist.get_rank() == 0:
