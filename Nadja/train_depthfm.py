...
def parse_args():
    parser = argparse.ArgumentParser("FM depth estimation")
...
def main():
    ...
    """ Setup model """
    module = TrainerModuleLatentDepthFM(
        fm_cfg=cfg.model.fm,
        start_from_noise=cfg.model.get("start_from_noise", False),
        noising_step=cfg.model.get("noising_step", -1),
        concat_context=cfg.model.get("concat_context", False),
        first_stage_cfg=cfg.model.get("first_stage", None),
        scale_factor=cfg.model.get("scale_factor", 1.0),
        sample_train_posterior=cfg.model.get("sample_train_posterior", True),
        normalization_fn=cfg.model.get("normalization_fn", "identity"),
        # ema stuff
        ...
        # conditioning
        cond_stage_cfg=cfg.model.get("cond_stage", None),
        cond_dropout=cfg.model.get("cond_dropout", 0.0),
        cfg_scale=cfg.model.get("cfg_scale", 1.0),
    )
    ...
    some_info = {
        ...
        'Start from noise': cfg.model.get("start_from_noise", False),
        'Noising step': cfg.model.get("noising_step", -1),
        'Concat context': cfg.model.get("concat_context", False),
